...
while  (
       ip_add.split('.')[0].isdigit() or
       ip_add.split('.')[1].isdigit() or
       ip_add.split('.')[2].isdigit() or
       ip_add.split('.')[3].isdigit() or
       int(ip_add.split('.')[0]) in range(256) or
       int(ip_add.split('.')[1]) in range(256)  or
       int(ip_add.split('.')[2]) in range(256)  or
       int(ip_add.split('.')[3]) in range(256)  or
       len(ip_add.split('.')) == 4
       ): 
    ...
    print('IP_address is wrong, try gain')
